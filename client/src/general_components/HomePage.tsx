...
const HomePage = () => {
  ...
  let joinLobby = async (newCode: string, clickedJoin: boolean) => {
    ...
    if (name == "" && clickedJoin) {
      ...
      try {
        ...
        const res : AxiosResponse<Exists> = await axiosConfig.get(`/${newCode}`, { signal: controller.signal });
        clearTimeout(timer);
        ...
        if (!res.data.exists)
          ...
          let timer = setTimeout(() => { controller.abort()}, 1000)
          let res : AxiosExistsRes = await axiosConfig.get(`/${newCode}/user`, { params: {name: name}});
          clearTimeout(timer);
  ...
  let tryToCreateLobby = async (newCode: string) => {
    try {
      // abort post request after 1 second. will enter catch statement if aborted
      let timer = setTimeout(() => { controller.abort()}, 1000)
      const res : AxiosResponse<Success> = await axiosConfig.post(`/${newCode}`, null,  { signal: controller.signal });
      clearTimeout(timer);
  ...
  return (
    <div>
      <div className="row min-vh-100">
        ...
        <div className="col-sm my-auto float-none text-center pt-5">
          <img
            src={whiteLogo}
            alt="..."
            className="img m-auto float-none mb-4"
            width="150"
          />
          <form>
            <div className="form-group w-50 text-center m-auto float-none ">
              <input
                type="text"
